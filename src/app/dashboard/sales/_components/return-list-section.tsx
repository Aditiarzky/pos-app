...
export function ReturnListSection() {
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      toast.error((error as ApiResponse).error || "Gagal membatalkan retur");
    }
  };
                ...
                {customerReturns?.map((ret, idx) => (
                  <TableRow key={ret.id}>
                    ...
                    <TableCell>
                      ...
                      >
                        {ret.compensationType.replace("_", " ")}
                      </Badge>
                    </TableCell>
            ...
            {customerReturns?.map((ret) => (
              <Card key={ret.id} className="p-4 space-y-3">
                <div className="flex justify-between items-start">
                  <div>
                    <h4 className="font-bold text-sm">{ret.returnNumber}</h4>
                    <p className="text-xs text-muted-foreground">
                      {formatDate(ret.createdAt || new Date())}
                    </p>
                  </div>
                  <Badge variant="outline">{ret.compensationType}</Badge>
                </div>
