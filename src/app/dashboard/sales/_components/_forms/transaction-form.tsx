...
export function TransactionForm({ onSuccess }: TransactionFormProps) {
  ...
  useEffect(() => {
    // Sinkronisasi dengan bottom-navbar.tsx
                ...
                <div className="space-y-2 pt-1">
                  ...
                  <div className="relative">
                    ...
                    <CurrencyInput
                      ...
                      onChange={(val) =>
                        form.setValue("totalPaid", Number(val))
                      }
                    />
                  </div>
            ...
            {isQris && (
              <div className="p-3 bg-blue-50 dark:bg-blue-950/30 rounded-lg border border-blue-200 dark:border-blue-800 text-sm space-y-1">
                ...
                <p className="text-xs text-blue-600/80 dark:text-blue-400/80">
                  QR code akan ditampilkan setelah tombol diproses. Customer
                  scan menggunakan aplikasi m-banking atau dompet digital.
                </p>
              </div>
            )}
  ...
  return (
    <>
      ...
      <Sheet open={isMobileCheckoutOpen} onOpenChange={setIsMobileCheckoutOpen}>
        <SheetContent
          side="bottom"
          className="h-[85vh] px-4 pt-10 pb-6 rounded-t-3xl border-t shadow-2xl overflow-y-auto w-full max-w-full"
        >
          <SheetTitle hidden>Checkout</SheetTitle>
          <div className="absolute left-1/2 -translate-x-1/2 top-3 h-1.5 w-12 rounded-full bg-muted-foreground/20" />
          {renderPaymentUI(true)}
        </SheetContent>
      </Sheet>
      ...
      <div className="relative grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8 min-h-[600px] items-start pb-24 lg:pb-0">
        ...
        <div className="hidden lg:block lg:col-span-1 sticky top-4 md:top-6">
          {renderPaymentUI(false)}
        </div>

        {/* ── MOBILE STICKY BOTTOM BAR ── */}
        <div
          className={cn(
            "lg:hidden fixed left-0 right-0 z-40 transition-all duration-300 transform",
            isNavVisible ? "bottom-[60px] md:bottom-16" : "bottom-0",
            fields.length === 0
              ? "translate-y-full opacity-0 pointer-events-none"
              : "translate-y-0 opacity-100",
          )}
        >
          <div className="bg-background/95 backdrop-blur-md border-t border-muted shadow-[0_-8px_20px_rgba(0,0,0,0.1)] p-4 mx-auto container md:rounded-t-2xl">
            <div className="flex items-center justify-between gap-4">
              <div className="grid gap-0.5">
                <span className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground opacity-60 leading-none">
                  Total Tagihan
                </span>
              ...
              <Button
                size="lg"
                type="button"
                className="flex-[0.8] h-12 font-bold uppercase tracking-widest text-xs rounded-xl shadow-md bg-primary/90 hover:bg-primary"
