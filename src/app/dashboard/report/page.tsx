  ...
  const rows = [
    {
      ...
      prev: formatCurrency(summary.prevTotalSales ?? 0),
      note: `${summary.totalSalesTransactions} transaksi penjualan`,
    },
    {
      label: "Total Pembelian",
      value: formatCurrency(summary.totalPurchases),
      prev: formatCurrency(summary.prevTotalPurchases ?? 0),
      note: `${summary.totalPurchaseTransactions} transaksi pembelian`,
    },
    {
      label: "Arus Kas Bersih",
      value: formatCurrency(netFlow),
      prev: "—",
      note: "Pendapatan − Pembelian",
      bold: true,
    },
    {
      label: "Laba Kotor",
      value: formatCurrency(summary.grossProfit),
      prev: formatCurrency(summary.prevGrossProfit ?? 0),
      note: "Pendapatan − HPP",
      bold: true,
    },
    {
      ...
      prev: "—",
      note: "Dinormalisasi ke periode ini",
    },
    {
      label: "Pajak",
      value: `−${formatCurrency(summary.totalTax)}`,
      prev: "—",
      note: "Semua pajak aktif",
    },
    {
      label: "Laba Bersih",
      value: formatCurrency(summary.netProfit),
      prev: "—",
      note: "Laba Kotor − Biaya Ops − Pajak",
      bold: true,
    },
    {
      label: "Jumlah Transaksi",
      value: String(summary.totalTransactions),
      prev: String(summary.prevTotalTransactions ?? 0),
        ...
        <tbody>
          {rows.map((row, i) => (
            ...
            >
              ...
              <td className="border border-gray-300 px-3 py-2 text-right tabular-nums text-gray-500">
                {row.prev}
              </td>
        ...
        <tbody>
          {products.map((item, i) => (
            ...
            >
              ...
              <td className="border border-gray-300 px-3 py-2 text-right tabular-nums font-semibold">
        ...
        {(!breakdown ||
          ...
          <div className="flex items-start gap-2 rounded-lg bg-amber-50 border border-amber-200 p-2.5">
            ...
            <p className="text-[11px] text-amber-700">
              Belum ada biaya operasional atau pajak yang dikonfigurasi. Laba
              bersih sama dengan laba kotor.
            </p>
          </div>
        )}
      </CardContent>
...
export function ReportContent() {
  ...
  const topCategories = reportQuery.data?.data?.topCategories ?? [];
  ...
  const dailySummary = useMemo(() => {
    ...
    return fillDailyGaps(raw, appliedFilter.startDate, appliedFilter.endDate, [
      "totalSales",
      "totalPurchases",
    ]) as ChartDataForInteractiveChart[];
    ...
    <>
      ...
      <div className="container mx-auto space-y-4">
        ...
        >
          ...
          <PrintCategoriesTable categories={topCategories} />
          ...
          {reportQuery.isError && (
            <Alert variant="destructive" className="no-print">
              ...
              <AlertDescription>
                Terjadi kesalahan. Coba ulangi beberapa saat lagi.
              </AlertDescription>
            </Alert>
          )}
          ...
          {/* Main Content Grid: Charts + Tables + Sidebar */}
          <div className="no-print grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            {/* Left Column: Interactive Chart + Detailed Tables */}
            <div className="lg:col-span-2 space-y-6">
                      ...
                      {reportQuery.isLoading ? (
                        ...
                        topProducts.map((item, idx) => (
                          ...
                          >
                            ...
                            <TableCell className="text-right text-primary font-bold text-sm tabular-nums">
                              {formatCurrency(item.revenue)}
                            </TableCell>
                            <TableCell className="text-right pr-6 text-emerald-600 font-semibold text-sm tabular-nums">
                              {formatCurrency(item.grossProfit)}
                            </TableCell>
                          </TableRow>
                        ))
              ...
              {/* Kategori terlaris */}
              <Card className="shadow-md pt-0 gap-0 no-print">
                <CardHeader className="border-b bg-muted/20">
                  <CardTitle className="flex items-center justify-between pt-6">
                    <div className="flex items-center gap-2">
            ...
            {/* Right Column: Pie Chart + Financial Summaries */}
